...
class MinimalNode(Node):
    ...
    def timer_callback(self):
        ...
        if distance > lookahead_distance:
            lookahead_angle = self.normalize_angle(math.atan2(dy, dx) - self.pose.theta)
            angular_z = 2.0 * math.sin(lookahead_angle) / lookahead_distance
            linear_x = min(distance, 2.0)
        
        angular_z = max(min(heading_error, 2.0), -2.0)
        linear_x = min(distance, 2.0)
